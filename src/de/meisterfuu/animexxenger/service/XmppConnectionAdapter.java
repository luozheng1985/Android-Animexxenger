 ...
 */
package de.meisterfuu.animexxenger.service;
...
import de.meisterfuu.animexxenger.BeemService;
import de.meisterfuu.animexxenger.R;
import de.meisterfuu.animexxenger.service.aidl.IBeemConnectionListener;
import de.meisterfuu.animexxenger.service.aidl.IChatManager;
import de.meisterfuu.animexxenger.service.aidl.IRoster;
import de.meisterfuu.animexxenger.service.aidl.IXmppConnection;
import de.meisterfuu.animexxenger.smack.avatar.AvatarCache;
import de.meisterfuu.animexxenger.smack.avatar.AvatarListener;
import de.meisterfuu.animexxenger.smack.avatar.AvatarMetadataExtension;
...
public class XmppConnectionAdapter extends IXmppConnection.Stub {
	 ...
	 */
	public XmppConnectionAdapter(final ConnectionConfiguration config, final String login, final String password, final BeemService service) {
	 ...
	 *             the background service associated with the connection.
	 */
	public XmppConnectionAdapter(final String serviceName, final String login, final String password, final BeemService service) {
	 ...
	 *             the background service associated with the connection.
	 */
	public XmppConnectionAdapter(final XMPPConnection con, final String login, final String password, final BeemService service) {
		mAdaptee = con;
		...
		mResource = mPref.getString(BeemApplication.CONNECTION_RESOURCE_KEY, "Android Animexxenger");
	}
			...
			try {
				...
				try {
					...
					String str = mService.getResources().getString(
							mService.getResources().getIdentifier(e.getXMPPError().getCondition().replace("-", "_"), "string", "de.meisterfuu.animexxenger"));
					mErrorMsg = str;
		...
		public void reconnectionSuccessful() {
			...
			mAdaptee.addPacketListener(new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					Notification notif = new Notification(android.R.drawable.stat_notify_more, mService.getString(R.string.AcceptContactRequest, from), System.currentTimeMillis());
	...
	private class SubscribePacketListener implements PacketListener {
		...
		public void processPacket(Packet packet) {
			...
			Notification notification = new Notification(android.R.drawable.stat_notify_more, mService.getString(R.string.AcceptContactRequest, from), System.currentTimeMillis());
			notification.flags = Notification.FLAG_AUTO_CANCEL;
			Intent intent = new Intent(mService, Subscription.class);
			intent.setData(Contact.makeXmppUri(from));
			notification
					.setLatestEventInfo(mService, from, mService.getString(R.string.AcceptContactRequestFrom, from), PendingIntent.getActivity(mService, 0, intent, PendingIntent.FLAG_ONE_SHOT));
			int id = p.hashCode();
